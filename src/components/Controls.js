...
export default function Controls(props) {
    ...
    return (
        <div>
            <button className="inline" onClick={props.functionPlay} id="start_stop">{props.play ? pause : play}</button>
            <button className="inline" onClick={props.functionRestart} id="reset">{reset}</button>
        </div>
    )
}
...
export default function BreakLength(props) {
    ...
    useEffect(() => {
        if (props.sessOrBreak) {
        props.setRemTime(props.time*60)
        }
    }, [props.time])
    ...
    return(
        <div className="length-control">
            ...
            <button id="break-decrement" onClick={decrease} className="inline">{down}</button>
            <p id="break-length" className="inline" >{props.time}</p>
            <button id="break-increment" onClick={increase} className="inline">{up}</button>
        </div>
    )
}